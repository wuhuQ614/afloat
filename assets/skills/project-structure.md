...
- **核心依赖**：http / shared\_preferences / window\_manager（仅桌面，Android 禁用）/ flutter\_tts（朗读）/ audioplayers / flutter\_inappwebview（浏览器）/ file\_picker / pdf / excel / flutter\_local\_notifications（课程提醒）
...
| `storage.dart`                                   | shared\_preferences 封装，全部配置/数据的持久化                                                                   |
...
| `wechat_service.dart`                            | 微信绑定（weixin\_clawbot，扫码后自动应答回传）                                                                      |
...
| `timetable_page.dart` / `timetable_settings_dialog.dart` / `timetable_format_doc.dart` / `edit_timetable_dialog.dart` | 课表功能（含导入/编辑）                                     |
...
> Agent 跳转统一用原生工具 `goto_page`（参数如 learn / exam / timetable / report / dictionary / wrong / favorite / records / dictation / grammar / maimemo / browser / game / multi\_expert / debate / source）。
...
- **技能商店**：`skillCatalog` 只把"名称+描述"目录注入系统提示词，全文仅 skill/load\_skill 命中时加载（渐进式披露）；`skillsDisabledIds` / `customSkills` 持久化
...
- **DeepSeek V4**：控制思考必须用 `{"type":"disabled"|"enabled"}`（api\_service.dart 的 noThinkingParams/thinkingParams 已处理，state.dart 的 `_noThinkingParams` 同步）；布尔 `enable_thinking` 对 V4 无效
...
- **在线更新**：远端仓库根目录 `update.json`（多源清单见 update\_service.dart）+ GitHub Releases 安装包；应用启动约 6 秒后静默检查，有新版才弹窗
...
3. Android 平台禁止引入 Windows 专用插件（如 window\_manager）
...
9. 出题接口一律 200k max\_tokens（普通对话不受此约束）
