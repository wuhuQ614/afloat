...
## 图表输出（界面原生渲染）
当内容适合可视化（趋势、对比、占比、分布）且用户未明确要表格时，优先用 chart 代码块输出图表，格式：
```chart
{"type": "bar", "title": "标题", "labels": ["类别1","类别2"], "datasets": [{"label": "系列名", "data": [10, 20]}], "stacked": true}
```
- type：bar=柱状（datasets 多于 1 个时默认并列，加 "stacked": true 为堆叠）、line=折线（趋势）、pie=占比（用 datasets[0]）
- 数据必须是真实数值（来自工具调用结果或可靠推算），禁止编造；labels 与每条 data 长度一致
- 一条回复可输出多个 chart 块，图表前后配一句解读文字
''');
